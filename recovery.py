###
# credits to M. Wassmer for the original script
###
...
if "Data" in config_sections:
    ...
    original_publication = False#config.section_("Data").section_("publication")
...
new_name = "recovery_"+original_request_name
if len(new_name) > 100: new_name = new_name[:100]
config.section_("General").__setattr__("requestName", new_name)
...
officialLumiMask = None
try:
    officialLumiMask = getLumiListInValidFiles(dataset=original_input_dataset, dbsurl=dbs_instance)
except:
    officialLumiMask = LumiList(crab_project_dir + "/results/inputDatasetLumis.json")
...
cfg_string = str(config)
cfg_string = cfg_string.replace("from WMCore.Configuration import Configuration\nconfig = Configuration()", 
    "import os\nfrom CRABClient.UserUtilities import config\nconfig = config()")
cfg_string+="config.Data.unitsPerJob = 240"
try:
    with open(recovery_config,"w") as f_out:
        f_out.write(cfg_string)
except IOError:
